...
func (self *StakeState) getBlockRecords(getter decedb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share, pools []*StakePool, err error) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func (self *StakeState) getShares(getter decedb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share, err error) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func (self *StakeState) ProcessBeforeApply(bc blockChain, header *types.Header) (err error) {
	// if header.Number.Uint64() == deceparam.SIP8() {
	// 	InitAVLTree(self)
	// 	// NewTree(self, header.Number.Uint64()).Midtraverse()
	// }

	// if header.Number.Uint64() > deceparam.SIP8() && header.Number.Uint64()%100 == 0 {
	// 	NewTree(self, header.Number.Uint64()).Midtraverse()
	// }
...
func (self *StakeState) statisticsByWindow(header *types.Header, bc blockChain) error {
	...
	if preHeader.Number.Uint64() >= statisticsMissWindow {
