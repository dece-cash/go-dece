...
import (
	...
	"github.com/dece-cash/go-dece/internal/jsre"
	"github.com/dece-cash/go-dece/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var dece = web3.dece; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = dece.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		dece, err := c.jsre.Get("dece")
		if err != nil {
			return err
		}

		if obj := dece.Object(); obj != nil {
			if _, err = c.jsre.Run(`jeth.sendTransaction = dece.sendTransaction;`); err != nil {
				return fmt.Errorf("dece.sendTransaction: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.createPkg = dece.createPkg;`); err != nil {
				return fmt.Errorf("dece.createPkg: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.transferPkg = dece.transferPkg;`); err != nil {
				return fmt.Errorf("dece.transferPkg: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.closePkg = dece.closePkg;`); err != nil {
				return fmt.Errorf("dece.closePkg: %v", err)
			}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines dece.getBalance(dece.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	fmt.Fprintf(c.printer, "Welcome to the gece JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + dece.coinbase);
		console.log("at block: " + dece.blockNumber + " (" + new Date(1000 * dece.getBlock(dece.blockNumber).timestamp) + ")");
	...
	if apis, err := c.client.SupportedModules(); err == nil {
		...
		for api, version := range apis {
			if api == "dece" {
				modules = append(modules, fmt.Sprintf("%s:%s", "dece", version))
			} else {
