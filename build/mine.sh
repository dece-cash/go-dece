...
PATTERN_ATTACH_PROCESS="gece.*attach"
PATTERN_MAIN_PROCESS=".*gece.*datadir.*port.*"
PATTERN_ACCOUNTS="\[*\]"
...
loadDefaultParams() {
   ...
   echo "export DATADIR=~/.datadir"	> $geceCFG
   echo "export SERVERPORT=60601" >> $geceCFG
   echo "export RPCPORT=8545" >> $geceCFG
}
killProcess() {
    if [[ -z $1  ]]; then
...
verifyServerPort(){
    ...
    if [ $portToCheck != $1 ]; then
        ...
        set -o xtrace
        sed -i -e "s/${portName}=$1/${portName}=$portToCheck/g" ~/.gececfg
        set +x
...
makeAccount(){
    ...
    echo "personal.newAccount()" > /tmp/gece-input
    echo "${PASSWORD}" > /tmp/gece-input
    echo "${PASSWORD}" > /tmp/gece-input
    while read lineA; do
        if [ -z ${lineA} ]; then
            echo "finished" >&2
        ...
        if read lineA; then
            ...
            if [ $length == 1 -a "${LINEA_NO_WHITESPACE}" == ">" ]  ; then
             #    echo "it is a hint" >&2
                ...
                echo "meet fatal error, please check the log in ~/working/gece.log" >&2
                exit -1
            fi
            ...
            if [ "$lineB" == "BEGIN_PASSWORD:true" ]; then
              ...
              #  echo "${PASSWORD}"
                ...
                if [ $length -le 10 ]; then
                    ...
                    printf '%s\n' "personal.newAccount()" >/tmp/gece-input
                    sleep 5
                    continue 
                fi 
                ...
                printf 'miner.setSerobase("%s")\n' "${ACCOUNT}" >/tmp/gece-input &
                echo 'miner.start()' >/tmp/gece-input &
                exit 
...
if [ "${result}" == "-1" ]; then
    ...
    set -o xtrace
    nohup ./bin/gece --alpha --datadir=${DATADIR} --rpc --rpcport ${RPCPORT}  --port ${SERVERPORT} --rpccorsdomain "*" &> ~/.log/gece.log &
    set +x
    sleep 30
fi
result=$(checkProcessWorking "$PATTERN_MAIN_PROCESS")
if [ "${result}" == "-1" ]; then
    echo "Please check why the main process of gece not running with log in ~/working/"
    exit
fi 
if [ -p "/tmp/gece-input" ] || [ -f "/tmp/gece-input" ]; then
    rm /tmp/gece-input
fi
if [ -p "/tmp/gece-output" ] || [ -f "/tmp/gece-output" ]; then
    rm /tmp/gece-output
fi
if [ -p "./localio" ] || [ -f "./localio" ]; then
    rm ./localio
fi
mkfifo -Z --mode='a=rwx' /tmp/gece-input
mkfifo -Z --mode='a=rwx' /tmp/gece-output
mkfifo -Z --mode='a=rwx' ./localio
cat /tmp/gece-input|./bin/gece --datadir=~/.datadir attach > /tmp/gece-output &
exec 9<> /tmp/gece-input
readingFromOutput &
backgroundPid=$!
printf '%s\n' 'dece.accounts' >/tmp/gece-input
