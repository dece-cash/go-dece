...
cmd="bin/gece --mineMode --config ${CONFIG_PATH} --unlock ${VOTER} --password ${VOTER_PASSWORD_PATH} --datadir ${DATADIR_OPTION} --port ${DEFAULT_PORT} ${KEYSTORE_OPTION}"
mkdir -p $LOGDIR

echo $cmd
current=`date "+%Y-%m-%d"`
logName="posgece_$current.log"
sh stop.sh
