...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gece is "gece". If no
...
func (c *Config) NodeName() string {
	...
	if name == "gece" || name == "gece-testnet" {
		name = "Gece"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gece" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gece 1.4 are used if they exist.
	if c.name() == "gece" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gece" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-dece-keystore")
		ephemeral = keydir
	}
