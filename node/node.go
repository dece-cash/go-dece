...
func (n *Node) OpenDatabase(name string, cache, handles int) (decedb.Database, error) {
	if n.config.DataDir == "" {
		return decedb.NewMemDatabase(), nil
	}
	return decedb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
