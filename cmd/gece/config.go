...
type decestatsConfig struct {
	URL string `toml:",omitempty"`
}

type deceConfig struct {
	Sero      dece.Config
	Node      node.Config
	Serostats decestatsConfig
	// Dashboard dashboard.Config
}

func loadConfig(file string, cfg *deceConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "decer", "shh")
	cfg.WSModules = append(cfg.WSModules, "dece", "shh")
	cfg.IPCPath = "gece.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, deceConfig) {
	// Load defaults.
	cfg := deceConfig{
		Sero:      dece.DefaultConfig,
