...
// Package utils contains internal helper functions for go-dece commands.
package utils
...
import (
	...
	"github.com/dece-cash/go-dece/common/fdlimit"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: dece.DefaultConfig.NetworkId,
	}
	...
	OfflineFlag = cli.BoolFlag{
		Name:  "offline",
		Usage: "Use gece as the offline mode",
	}
	...
	defaultSyncMode = dece.DefaultConfig.SyncMode
	...
	EthashCachesInMemoryFlag = cli.IntFlag{
		...
		Value: dece.DefaultConfig.Ethash.CachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "ethash.cachesondisk",
		Usage: "Number of recent ethash caches to keep on disk (16MB each)",
		Value: dece.DefaultConfig.Ethash.CachesOnDisk,
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "ethash.dagdir",
		Usage: "Directory to store the ethash mining DAGs (default = inside home folder)",
		Value: DirectoryString{dece.DefaultConfig.Ethash.DatasetDir},
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "ethash.dagsinmem",
		Usage: "Number of recent ethash mining DAGs to keep in memory (1+GB each)",
		Value: dece.DefaultConfig.Ethash.DatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "ethash.dagsondisk",
		Usage: "Number of recent ethash mining DAGs to keep on disk (1+GB each)",
		Value: dece.DefaultConfig.Ethash.DatasetsOnDisk,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: dece.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: dece.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: dece.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: dece.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: dece.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: dece.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: dece.DefaultConfig.GasPrice,
	}
	...
	SeroStatsURLFlag = cli.StringFlag{
		Name:  "decestats",
		Usage: "Reporting URL of a decestats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: dece.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: dece.DefaultConfig.GPO.Percentile,
	}
	// Metrics flags
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("-------------------------------------------------------------------")
	log.Warn("Referring to accounts by order in the keystore folder is dangeceus!")
	log.Warn("This functionality is deprecated and will be removed in the future!")
	log.Warn("Please use explicit addresses! (can search via `gece account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "DECE", cfg.MaxPeers)
	...
	if ctx.GlobalIsSet(ConfirmedBlockFlag.Name) {
		...
		if balanceDelay >= 0 {
			deceparam.InitComfirmedBlock(balanceDelay)
		}
	}
	...
	if ctx.GlobalIsSet(ResetBlockNumber.Name) {
		...
		deceparam.InitCurrentBlockNumber(blockNumber)
	}
	...
	if ctx.GlobalBool(DeveloperFlag.Name) {
		...
		deceparam.Init_Dev(true)
	}
	if ctx.GlobalBool(OfflineFlag.Name) {
		deceparam.Init_Offline(true)
	}
...
func setEthash(ctx *cli.Context, cfg *dece.Config) {
...
// SetSeroConfig applies dece-related command line flags to the config.
func SetSeroConfig(ctx *cli.Context, stack *node.Node, cfg *dece.Config) {
	...
	if ctx.GlobalIsSet(ExchangeValueStrFlag.Name) {
		deceparam.InitExchangeValueStr(true)
	}
...
// RegisterEthService adds an Dece client to the stack.
func RegisterEthService(stack *node.Node, cfg *dece.Config) {
	err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		fullNode, err := dece.New(ctx, cfg)
		return fullNode, err
	})
	if err != nil {
		Fatalf("Failed to register the Dece service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gece.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) decedb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb decedb.Database) {
	var err error
	...
	if !ctx.GlobalBool(FakePoWFlag.Name) {
		engine = ethash.New(ethash.Config{
			CacheDir:       stack.ResolvePath(dece.DefaultConfig.Ethash.CacheDir),
			CachesInMem:    dece.DefaultConfig.Ethash.CachesInMem,
			CachesOnDisk:   dece.DefaultConfig.Ethash.CachesOnDisk,
			DatasetDir:     stack.ResolvePath(dece.DefaultConfig.Ethash.DatasetDir),
			DatasetsInMem:  dece.DefaultConfig.Ethash.DatasetsInMem,
			DatasetsOnDisk: dece.DefaultConfig.Ethash.DatasetsOnDisk,
		})
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	...
	cache := &core.CacheConfig{
		...
		TrieNodeLimit: dece.DefaultConfig.TrieCache,
		TrieTimeLimit: dece.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheGCFlag.Name) {
...
// new format.
//
// e.g. gece account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gece --keystore /tmp/mykeystore --lightkdf account new
//
