...
// PublicSeroAPI provides an API to access Dece full node-related
// information.
type PublicSeroAPI struct {
	e *Dece
}

// NewPublicSeroAPI creates a new Dece protocol API for full nodes.
func NewPublicSeroAPI(e *Dece) *PublicSeroAPI {
...
func NewPublicMinerAPI(e *Dece) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Dece) *PrivateMinerAPI {
...
// SetSerobase sets the decebase of the miner
func (api *PrivateMinerAPI) SetSerobase(decebase address.MixBase58Adrress) bool {
	api.s.SetSerobase(decebase)
	return true
}
...
// PrivateAdminAPI is the collection of Dece full node-related APIs
...
// admin methods of the Dece service.
func NewPrivateAdminAPI(eth *Dece) *PrivateAdminAPI {
...
// PublicDebugAPI is the collection of Dece full node APIs exposed
...
// related public debug methods of the Dece service.
func NewPublicDebugAPI(eth *Dece) *PublicDebugAPI {
	return &PublicDebugAPI{dece: eth}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.dece.miner.Pending()
		return stateDb.RawDump(), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.dece.blockchain.CurrentBlock()
	} else {
		block = api.dece.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.dece.BlockChain().StateAt(block.Header())
	if err != nil {
...
// PrivateDebugAPI is the collection of Dece full node APIs exposed over
...
// private debug methods of the Dece service.
func NewPrivateDebugAPI(config *params.ChainConfig, eth *Dece) *PrivateDebugAPI {
