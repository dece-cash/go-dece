...
import (
	...
	"github.com/dece-cash/go-dece/dece/fetcher"
...
// NewProtocolManager returns a new Dece sub protocol manager. The Dece sub protocol manages peers capable
// with the Dece network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, voter shareVoter, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb decedb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < dece63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Dece peer", "peer", id)

	// Unregister the peer from the downloader and Dece peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Dece protocol")
	...
	log.Info("Dece protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an dece peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Dece peer connected", "name", p.Name())

	// Execute the Dece handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Dece handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Dece peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Dece message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= dece63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= dece63 && msg.Code == NodeDataMsg:
	...
	case p.version >= dece63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= dece63 && msg.Code == ReceiptsMsg:
...
// NodeInfo represents a short summary of the Dece sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Dece network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
