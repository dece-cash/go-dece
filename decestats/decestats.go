...
// Package decestats implements the network stats reporting service.
package decestats
...
import (
	...
	"github.com/dece-cash/go-dece/common/mclock"
	...
	//"github.com/dece-cash/go-dece/lesdeprecate"
...
// Service implements an Dece netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server // Peer-to-peer server to retrieve networking infos
	eth    *dece.Dece  // Full Dece service if monitoring a full node
	//lesdeprecate    *lesdeprecate.LightEthereum // Light Dece service if monitoring a light node
...
//func New(url string, ethServ *dece.Dece, lesServ *lesdeprecate.LightEthereum) (*Service, error) {
func New(url string, ethServ *dece.Dece) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["dece"]; info != nil {
		network = fmt.Sprintf("%d", info.(*dece.NodeInfo).Network)
		protocol = fmt.Sprintf("dece/%d", dece.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the decestats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to decestats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to decestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to decestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to decestats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to decestats")
