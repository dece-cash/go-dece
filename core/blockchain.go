...
import (
	...
	"github.com/dece-cash/go-dece/zero/txtool/verify"
...
type BlockChain struct {
	...
	db     decedb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db decedb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, accountManager *accounts.Manager) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (self *State1BlockChain) GetDB() decedb.Database {
	return self.Bc.db
}
...
func (self *State1BlockChain) IsValid() bool {
	...
	if header.Number.Uint64() < deceparam.DefaultConfirmedBlock() {
		return false
	}
...
func (self *State1BlockChain) GetDeceGasLimit(to *common.Address, tfee *assets.Token, gas *big.Int) (gaslimit uint64, e error) {
	if state, err := self.Bc.State(); err != nil {
		e = err
		return
	} else {
		return state.GetDeceGasLimit(to, tfee, gas)
	}
}
...
func (bc *BlockChain) GetDB() decedb.Database {
	return bc.db
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if deceparam.DefaultCurrentBlock() > 0 {
		currentBlock = bc.GetBlockByNumber(deceparam.DefaultCurrentBlock())
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= decedb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - decedb.IdealBatchSize)
			}
...
func (bc *BlockChain) insertChain(chain types.Blocks, local bool) (int, []interface{}, []*types.Log, error) {
	...
	abort, results := bc.NewHeaderChecker(chain)
	defer close(abort)
