...
func makeTestState() (Database, common.Hash, []*testAccount) {
	...
	db := NewDatabase(decedb.NewMemDatabase())
	state, _ := New(db, nil)
	...
	for i := byte(0); i < 96; i++ {
		...
		obj.AddBalance("dece", big.NewInt(int64(11*i)))
...
// account array.
func checkStateAccounts(t *testing.T, db decedb.Database, root common.Hash, accounts []*testAccount) {
	...
	for i, acc := range accounts {
		if balance := state.GetBalance(acc.address, "dece"); balance.Cmp(acc.balance) != 0 {
...
func checkTrieConsistency(db decedb.Database, root common.Hash) error {
...
func checkStateConsistency(db decedb.Database, root common.Hash) error {
...
func TestEmptyStateSync(t *testing.T) {
	...
	if req := NewStateSync(empty, decedb.NewMemDatabase()).Missing(1); len(req) != 0 {
...
func TestIncompleteStateSync(t *testing.T) {
	...
	checkTrieConsistency(srcDb.TrieDB().DiskDB().(decedb.Database), srcRoot)
