...
func handleAllotTicket(d []byte, evm *EVM, contract *Contract, mem []byte) (common.Hash, uint64, error, bool) {
	...
	if strings.Contains(categoryName, "DECE") {
		return common.Hash{}, 0, fmt.Errorf("allotTicket error , contract : %s, error : %s", contract.Address(), "categoryName can not contains DECE"), false
	}
...
func handleIssueToken(d []byte, evm *EVM, contract *Contract, mem []byte) (bool, error) {
	...
	if strings.Contains(coinName, "DECE") {
		return false, fmt.Errorf("issueToken error , contract : %s, error : %s", contract.Address(), "coinName can not contains DECE")
	}
	...
	if address == (common.Address{}) {
		...
		if evm.StateDB.GetBalance(contract.Address(), "DECE").Cmp(fee) >= 0 {
			if !evm.StateDB.RegisterToken(contract.Address(), coinName) {
				...
				evm.StateDB.SubBalance(contract.Address(), "DECE", fee)
				asset := assets.Asset{Tkn: &assets.Token{
					Currency: *common.BytesToHash(common.LeftPadBytes([]byte("DECE"), 32)).HashToUint256(),
