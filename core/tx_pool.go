...
func (pool *TxPool) checkDescCmd(tx *stx.T, state *state.StateDB) (err error) {
	...
	if cmd.BuyShare != nil {
		...
		if cmd.RegistPool.FeeRate > deceparam.HIGHEST_STAKING_NODE_FEE_RATE {
			err = fmt.Errorf("registPool Vote fee must <= %v%%", deceparam.HIGHEST_STAKING_NODE_FEE_RATE/100)
			return
		}
		if cmd.RegistPool.FeeRate < deceparam.LOWEST_STAKING_NODE_FEE_RATE {
			err = fmt.Errorf("registPool Vote fee must >= %v%%", deceparam.LOWEST_STAKING_NODE_FEE_RATE/100)
			return
		}
