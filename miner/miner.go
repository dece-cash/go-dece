...
func New(dece Backend, config *params.ChainConfig, mux *event.TypeMux, voter voter, engine consensus.Engine) *Miner {
	miner := &Miner{
		dece:     dece,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, accounts.Account{}, voter, dece, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(dece.BlockChain(), engine))
	go miner.update()
