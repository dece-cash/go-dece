...
func newWorker(config *params.ChainConfig, engine consensus.Engine, account accounts.Account, voter voter, dece Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:      config,
		engine:      engine,
		eth:         dece,
		...
		chainDb:     dece.ChainDb(),
		recv:        make(chan *Result, resultQueueSize),
		powRecv:     make(chan *Result),
		posTaskCh:   make(chan *Result),
		chain:       dece.BlockChain(),
		proc:        dece.BlockChain().Validator(),
		coinbase:    account,
		agents:      make(map[Agent]struct{}),
		unconfirmed: newUnconfirmedBlocks(dece.BlockChain(), miningLogAtDepth),
		voter:       voter,
		pendingVote: newPendingVote(),
	}
	// Subscribe NewTxsEvent for tx pool
	worker.txsSub = dece.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	worker.voteSub = worker.voter.SubscribeWorkerVoteEvent(worker.voteCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = dece.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = dece.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
	go worker.voteLoop()
